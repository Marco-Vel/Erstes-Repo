...
def update_dateien_anzahl(as_name=True):
    dateien = [(f.name if as_name else f) for f in ordner.iterdir() if f.is_file()] 
    return dateien, len(dateien)
...
def notizen_ansehen():
    ...
    while True:
        dateien, len(dateien) = update_dateien_anzahl(as_name=False)
        if len(dateien) >= 1:
...
def notizen_loeschen():
    while True:
        dateien, len(dateien) = update_dateien_anzahl(as_name=True)
        if len(dateien) >= 1:
